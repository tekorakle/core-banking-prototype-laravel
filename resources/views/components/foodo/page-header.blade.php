...
<header class="flex flex-col xl:flex-row justify-between items-start xl:items-end gap-6 mb-8">
    <div>
        ...
        <h1 class="text-2xl lg:text-3xl font-black text-foodo-text tracking-tight">{{ $title }}</h1>
    ...
    <div class="flex items-center gap-3 bg-white p-1.5 rounded-xl border border-stone-200 shadow-sm">
        <div class="px-4 py-2 bg-stone-100 rounded-lg text-xs font-bold text-stone-600 cursor-pointer hover:bg-stone-200 transition-colors flex items-center gap-2">
            {{ $contextLabel }} <span class="material-symbols-outlined text-sm">expand_more</span>
        </div>
        ...
        <div class="h-6 w-px bg-stone-200"></div>
        <a href="{{ $ctaRoute ?? route('foodo.chat') }}" class="bg-foodo-surface-dark text-white px-5 py-2.5 rounded-lg text-xs font-bold flex items-center gap-2 hover:bg-black transition-colors shadow-lg shadow-stone-900/20">
            <span class="material-symbols-outlined text-sm text-foodo-primary-vibrant foodo-icon">{{ $ctaIcon }}</span>
            {{ $ctaLabel }}
        </a>
    </div>
</header>
